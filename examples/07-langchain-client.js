...
async function sendToServer(transcript) {
  const res = await fetch("/api/langchain-demo-07", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ message: transcript, persona: loadPersona(), chatId }),
  });
  if (!res.ok) {
...
function startRecognition() {
  ...
  recognition.onresult = async (event) => {
    ...
    if (finalTranscript.trim()) {
      ...
      try {
        const { output } = await sendToServer(userText);
